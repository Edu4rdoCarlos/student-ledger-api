...
import { DocumentApprovalDto, DocumentWithApprovalsDto, CourseInDefenseDto } from '../../dtos/response/defense-response.dto';
...
export const ApiDefenseCreatedResponse = () =>
  applyDecorators(
    ApiExtraModels(DefenseResponseDto, AdvisorInDefenseDto, StudentInDefenseDto, ExamBoardMemberResponseDto, DocumentWithApprovalsDto, DocumentApprovalDto, CourseInDefenseDto),
    ApiCreatedResponse({
      description: 'Defesa criada com sucesso. Inclui informações sobre local, status, banca examinadora (se fornecida), orientador, alunos, curso e versões de documentos.',
      schema: {
        type: 'object',
        properties: {
          data: {
...
export const ApiDefenseOkResponse = () =>
  applyDecorators(
    ApiExtraModels(DefenseResponseDto, AdvisorInDefenseDto, StudentInDefenseDto, ExamBoardMemberResponseDto, DocumentWithApprovalsDto, DocumentApprovalDto, CourseInDefenseDto),
    ApiOkResponse({
      description: 'Defesa encontrada com informações detalhadas. Inclui: título, data, local, nota final, resultado, status, orientador, estudantes, curso, banca examinadora, documentos com assinaturas/aprovações. Os dados retornados dependem da role do usuário: ADMIN/COORDINATOR têm acesso total, ADVISOR/STUDENT participantes veem dados completos, não-participantes veem dados limitados.',
      schema: {
        type: 'object',
        properties: {
          data: {
...
export const ApiDefenseListResponse = () =>
  applyDecorators(
    ...
    ApiOkResponse({
      description: 'Lista de defesas com informações básicas e metadados de paginação. Retorna apenas dados essenciais: título, data, status, resultado, orientador, alunos e curso. Para informações detalhadas (banca examinadora, documentos, emails), use o endpoint GET /defenses/:id. Suporta filtro por texto através do parâmetro "search" que busca em: título da defesa, local, nome do aluno e nome do orientador.',
      schema: {
        type: 'object',
        properties: {
          data: {
